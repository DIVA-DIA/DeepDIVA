...
class LDA_CIFAR(nn.Module):
    ...
    def __init__(self):
        ...
        # Sixth layer
        self.conv6 = nn.Sequential(  # in: 8x8x192 out: 4x4x320
            nn.Conv2d(in_channels=192, out_channels=320, kernel_size=2,
                      stride=2),
            nn.Softsign()
        )
        # FC layer1
        self.fc1 = nn.Sequential(
            nn.Linear(in_features=4 * 4 * 320, out_features=512),
            nn.Softsign()
        )
        # FC layer1
...
class LDA_simple(nn.Module):
    ...
    def __init__(self):
        ...
        # Classification layer
        self.cl = nn.Sequential(
            nn.Linear(in_features=32 * 32 * 16, out_features=10),
        )
